...
import { updateApiClient } from '@/lib/paymentClient';
...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const isOnSupportedChain = true; // We're always on Base Sepolia
  const currentChainName = 'Base Sepolia';
  const chainId = baseSepolia.id; // 84532

  // Update API client whenever wallet changes
  useEffect(() => {
  ...
  const connectWallet = useCallback(async () => {
    ...
    try {
      ...
      // Create viem wallet client for x402 payments
      const client = createWalletClient({
        account: accounts[0] as Hex,
        chain: baseSepolia,
        transport: custom(window.ethereum)
      ...
      console.log('✅ Wallet connected to Base Sepolia:', accounts[0]);
    } catch (err: any) {
